...
- **临时调试文件不入仓库根目录**：调试脚本/截图放 `C:\Users\PC\AppData\Local\Temp\opencode` 或 `data/`（已 gitignore），用后删除；任务正式截图走框架产物 `data/screenshots/`（gitignore 内）；任务失败截图是排障证据不是成功判定，别把截图里的提示当结论（2026-09-09 教训：误把限频响应截图当验证通过）
