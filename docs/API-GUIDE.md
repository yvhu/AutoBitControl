...
- **窗口页**：搜索框（按名字/窗口 ID 过滤）＋「同步比特浏览器」按钮（拉取比特客户端窗口列表入库）＋ 窗口表（今日成功/失败数、熔断计数与进度条、启用开关、操作列）。行内「立即跑」= 跑该窗口的全部启用任务；「复制ID」一键复制比特窗口 ID 到剪贴板；「详情」打开右侧 **Drawer 抽屉**，展示该窗口今日任务时间线（Timeline，含状态与错误）与「重置熔断」按钮。
