...
- **任务页**：任务按空投分组折叠展示（Collapse 分区，组标题为「分组名 · N 个任务」，默认全展开；未写 group 的任务归入末尾「未分组」区；全部任务都未分组时保持平铺网格）。组内任务卡片网格（每卡两列，行内卡片等高），卡片含任务名/key/分类徽章（签到/领水/铸币/其他）、钱包/并发/重试/验证码摘要、备注、来源页链接；备注超 3 行自动折叠，点「展开/收起」切换（行内卡片等高）；停用或已失效任务半透明显示。卡片开关写入本地库 `task_states` 表，切换**立即生效**（无需重启）；「立即触发」= 该任务在全部启用窗口跑一遍（在途时按钮禁用显示「运行中」）。
