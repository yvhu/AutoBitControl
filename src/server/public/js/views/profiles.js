...
export async function render() {
  ...
  document.querySelector('#profile-table').innerHTML = rows.map(p => {
    ...
    return `<tr>
      <td><div style="display:flex;align-items:center;gap:8px"><div class="avatar">${String(p.id).padStart(2,'0')}</div><div><div>${esc(p.name)}</div><div style="font-size:10px;color:#8E9CB3">${esc(p.bitbrowserId)}</div></div></div></td>
      <td><span style="color:#34D399">${okCount} ✓</span>${fail ? ` <span style="color:#F87171">${fail} ✗</span>` : ''}</td>
      <td><span style="color:${p.circuitBreakerCount > 0 ? '#FBBF24' : '#8E9CB3'};font-size:11px">${p.circuitBreakerCount}/${settings.circuitBreakerThreshold}</span></td>
...
export async function openDrawer(id) {
  ...
  document.querySelector('#drawer-body').innerHTML = `
    <div style="border-left:2px solid #26324A;padding-left:14px;display:flex;flex-direction:column;gap:10px;margin-bottom:12px">
      ${mine.length ? mine.map(r => {
        const [cls] = PILLS[r.status] ?? ['skip']
        const dot = { ok: '#34D399', fail: '#F87171', cap: '#38BDF8', run: '#FBBF24', skip: '#BAC5D9' }[cls]
      ...
      }).join('') : '<div style="color:#8E9CB3">今日暂无任务记录</div>'}
    </div>
    <div style="font-size:12px;color:#BAC5D9;display:flex;gap:8px;align-items:center">
