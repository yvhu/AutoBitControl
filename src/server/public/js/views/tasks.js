...
const CATEGORY_BADGE = { checkin: ['签到', '#34D399'], faucet: ['领水', '#38BDF8'], mint: ['铸币', '#FBBF24'], other: ['其他', '#BAC5D9'] }
...
export async function render() {
  ...
  document.querySelector('#task-cards').innerHTML = tasks.map(t => {
    const icon = WALLET_ICON[t.wallet] ?? '<div class="wallet-ico" style="background:#3D4C6622">▣</div>'
    ...
    const cat = CATEGORY_BADGE[t.category] ?? ['其他', '#BAC5D9']
    return `<div class="task-card" style="${t.deprecated ? 'opacity:.45' : t.enabled === false ? 'opacity:.45' : ''}">
      ${icon}
      <div style="flex:1">
        <div style="font-weight:700;font-size:13px">${esc(t.name)} <span style="color:#8E9CB3;font-weight:400">${esc(t.key)}</span>
        ...
        ${t.note ? `<div class="meta" style="color:#BAC5D9">📝 ${esc(t.note)}</div>` : ''}
